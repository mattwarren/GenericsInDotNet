...
MethodTable * Verifier::s_pRuntimeTypeHandle;
MethodTable * Verifier::s_pRuntimeMethodHandle;
MethodTable * Verifier::s_pRuntimeFieldHandle;
MethodTable * Verifier::s_pRuntimeArgumentHandle;
MethodTable * Verifier::s_pTypedReference;
...
// @DEBUG InitializeLogging(); 
    ...
    m_MaxStackSlots      = ILHeader->GetMaxStack();  
    m_LocalSig           = ILHeader->LocalVarSig;   
    m_MaxLocals          = 0;
    m_CodeSize           = ILHeader->GetCodeSize();  
    ...
    {   
        ...
        m_pInternalImport->GetSigFromToken(ILHeader->GetLocalVarSigTok(), &cSig);
...
BOOL Verifier::CreateExceptionList(const COR_ILMETHOD_SECT_EH* ehInfo)
{
    ...
    for (i = 0; i < m_NumExceptions; i++)
    {
        ...
        c.dwTryXX           = ehClause->GetTryOffset();
        c.dwTryEndXX        = ehClause->GetTryOffset() + ehClause->GetTryLength();
        c.dwHandlerXX       = ehClause->GetHandlerOffset();
        c.dwHandlerEndXX    = ehClause->GetHandlerOffset() + ehClause->GetHandlerLength();
        c.eFlags            = ehClause->GetFlags();
        
        tok                 = ehClause->GetClassToken();
        ...
        if (c.eFlags & COR_ILEXCEPTION_CLAUSE_FILTER)
        {
            ...
            // try.. filter ... handler
            c.dwFilterXX = ehClause->GetFilterOffset();
            c.dwFilterEndXX = 0;
        ...
        switch (opcode)
        {
            ...
            case CEE_MKREFANY:
            {
                ...
                if (s_pTypedReference == NULL)
                    s_pTypedReference = g_Mscorlib.GetClass(CLASS__TYPED_REFERENCE);

                _ASSERTE(s_pTypedReference != NULL);

                NewItem.SetTypeHandle(TypeHandle(s_pTypedReference));
                (void) Push(&NewItem);
            ...
            case CEE_REFANYTYPE:
            {
                ...
                DesiredItem.SetTypeHandle(TypeHandle(s_pTypedReference));
                ...
                item.SetTypeHandle(TypeHandle(s_pRuntimeTypeHandle));
...
dequeueBB:
            ...
            case CEE_LDLOC_S:
            {
                ...
                // set init locals.
                if ((m_pILHeader->GetFlags() & CorILMethod_InitLocals) == 0)
                {
                    m_sError.dwFlags = VER_ERR_OPCODE_OFFSET;
                    SET_ERR_OPCODE_OFFSET();
...
error_bad_token:
            ...
            case CEE_ARGLIST:
            {
                ...
                if (s_pRuntimeArgumentHandle == NULL)
                    s_pRuntimeArgumentHandle = g_Mscorlib.GetClass(CLASS__ARGUMENT_HANDLE);

                _ASSERTE(s_pRuntimeArgumentHandle != NULL);

                Item item;

                item.SetTypeHandle(TypeHandle(s_pRuntimeArgumentHandle));
                ...
                switch (TypeFromToken(inline_operand))
                {
                    ...
                    case mdtMethodDef:
                    {
                        if (s_pRuntimeMethodHandle == NULL)
                            s_pRuntimeMethodHandle = g_Mscorlib.GetClass(CLASS__METHOD_HANDLE);
                        _ASSERTE(s_pRuntimeMethodHandle != NULL);
                    ...
                    DO_FIELD:
                        if (s_pRuntimeFieldHandle == NULL)
                            s_pRuntimeFieldHandle = g_Mscorlib.GetClass(CLASS__FIELD_HANDLE);
        
                        _ASSERTE(s_pRuntimeFieldHandle != NULL);
...
handle_ldfld:
                ...
                if (!fStatic)
                {
                    ...
                    RefItemValType.SetTypeHandle(pRefClass->GetMethodTable());
                    ...
                    if (!pInstance->CompatibleWith(&RefItem, m_pClassLoader) && 
                        !(!fLoadAddress && pInstance->CompatibleWith(&RefItemValType, m_pClassLoader) ) )
                    {
                        m_sError.dwFlags = 
                            (VER_ERR_ITEM_F|VER_ERR_ITEM_E|VER_ERR_OPCODE_OFFSET);
                        m_sError.sItemFound = pInstance->_GetItem();
...
DlgtCtorError:
        ...
        Item * pMethod = GetStack(0);   // Get the method pointer

        pFtn = pMethod->GetMethod();
...
#ifdef _DEBUG
    if (m_pILHeader->GetFlags() & CorILMethod_InitLocals)
