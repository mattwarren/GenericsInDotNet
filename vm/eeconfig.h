...
public:
    ...
    DWORD GetStressComCall ()               const {return dwStressComCall; }
#endif
    ...
    static DWORD GetConfigDWORD(LPCWSTR name, DWORD defValue,
...
#ifdef _DEBUG
    // GC alloc logging
    bool ShouldLogAlloc(const char *pClass) const { return pPerfTypesToLog && pPerfTypesToLog->IsInList(pClass);}
...
#define DEFAULT_TARGET_CODE_CACHE_SIZE 0x4000000  // 64 MB
...
public:
    HRESULT AppendConfigurationFile(LPCWSTR pszFileName, LPCWSTR version);
