...
#define TYPEINFO(enumName,className,size,gcType,isArray,isPrim,isFloat, isModifier) \
    { enumName, className, size, gcType, isArray, isPrim, isFloat, isModifier },
...
{
    m_Crst.Init("HardCodedMetaSig", CrstSigConvert);
    return TRUE;
}
				     ...
				     Substitution *pSubst) const
{
    ...
    switch(typ) {
	  ...
	  return(psig.GetTypeHandle(pModule, pThrowable, dontRestoreTypes, dontLoadTypes, approxTypes,
        ...
        {
            TypeHandle typeHandle;
            return typeHandle;       // For now type handles to value arrays unsupported
        }
        ...
        default:
            _ASSERTE(!"Bad type");
        case ELEMENT_TYPE_SENTINEL:     // just return null for ELEMENT_TYPE_SENTINEL
            ;
    }
    return TypeHandle();
}
...
BOOL CompareTypeTokens(mdToken tk1, mdToken tk2, Module *pModule1, Module *pModule2)
{
    ...
    if (TypeFromToken(tk1) == mdtTypeRef) 
    {
        ...
        if (enclosingTypeTk1 == mdTypeRefNil)
            enclosingTypeTk1 = mdTokenNil;       // something that is not a type token
    }
    else
    ...
    if (TypeFromToken(tk2) == mdtTypeRef) 
    {
        ...
        if (enclosingTypeTk2 == mdTypeRefNil)
            enclosingTypeTk2 = mdTokenNil;       // something that is not a type token
    }
    else 
    ...
    //////////////////////////////////////////////////////////////////////
    // OK, we have non-nested types, compare the assemblies 
    Assembly* pFoundAssembly1 = pModule1->GetAssembly();
    Assembly* pFoundAssembly2 = pModule2->GetAssembly();
    if (pFoundAssembly1 == pFoundAssembly2)
        return TRUE;

    // Note that we are loading the modules here.  Don't know if that is a real problem or not
    if (TypeFromToken(tk1) == mdtTypeRef) 
    {
        ...
        if (hr == CLDB_S_NULL) {
            ...
            TypeHandle typeHnd = pModule1->GetClassLoader()->LoadTypeHandle(&name1);
    ...
    if (TypeFromToken(tk2) == mdtTypeRef) 
    {
        ...
        if (hr == CLDB_S_NULL) {
            ...
            TypeHandle typeHnd = pModule2->GetClassLoader()->LoadTypeHandle(&name2);
                          ...
                          int   *pOffsetIntoArgumentRegisters)
{
    ...
    return FALSE;
#elif defined(_IA64_) || defined(_AMD64_) || defined(_PPC_) || defined(_SPARC_)
    return FALSE;
#else
...
// in the sig.
//------------------------------------------------------------------------
/*static*/ UINT MetaSig::NumVirtualFixedArgs(Module* pModule, PCCOR_SIGNATURE szMetaSig, BOOL fIsStatic)
{
    UINT result = 0;
    MetaSig msig(szMetaSig, pModule);

    if (!fIsStatic)
        result++;  // the this pointer
    if (msig.HasRetBuffArg())
        result++; // the return buffer address

    // What about the vararg cookie??
  ...
  TypeHandle *methodInst, BOOL fParamTypeArg, int* paramTypeReg)
{
    ...
    numregsused = NUM_ARGUMENT_REGISTERS;  // This forces IsArgumentInRegister(..) to return false
...
void MetaSig::ForceSigWalk(BOOL fIsStatic)
{
    ...
    m_cbSigSize = (UINT32)((PBYTE) p.GetPtr() - (PBYTE) m_pszMetaSig);
