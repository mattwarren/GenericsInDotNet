...
struct DebuggerIPCControlBlock
{
    ...
    // These are not HANDLE types - they're the "remotable" version which can be converted
    // in-process back into true HANDLEs by calling PAL_RemoteHandleToLocal()
    RHANDLE                    m_remoteRightSideEventAvailable;
    RHANDLE                    m_remoteRightSideEventRead;
    RHANDLE                    m_remoteLeftSideEventAvailable;
    RHANDLE                    m_remoteLeftSideEventRead;
    RHANDLE                    m_remoteRightSideProcessHandle;
    RHANDLE                    m_remoteLeftSideUnmanagedWaitEvent;
    RHANDLE                    m_remoteSyncThreadIsLockFree;
                 ...
                 HANDLE lsuwe, HANDLE stilf)
    {
        ...
        m_remoteRightSideEventAvailable = PAL_LocalHandleToRemote(m_rightSideEventAvailable);
	if (m_remoteRightSideEventAvailable == INVALID_HANDLE_VALUE) {
	    return E_OUTOFMEMORY;
	}
	m_remoteRightSideEventRead = PAL_LocalHandleToRemote(m_rightSideEventRead);
	if (m_remoteRightSideEventRead == INVALID_HANDLE_VALUE) {
	    return E_OUTOFMEMORY;
	}
	m_remoteLeftSideUnmanagedWaitEvent = PAL_LocalHandleToRemote(m_leftSideUnmanagedWaitEvent);
	if (m_remoteLeftSideUnmanagedWaitEvent == INVALID_HANDLE_VALUE) {
	    return E_OUTOFMEMORY;
	}
	m_remoteSyncThreadIsLockFree = PAL_LocalHandleToRemote(m_syncThreadIsLockFree);
	if (m_remoteSyncThreadIsLockFree == INVALID_HANDLE_VALUE) {
	    return E_OUTOFMEMORY;
	}
...
inline void SetDebuggerRegDisplayFromContext(DebuggerREGDISPLAY *pRegDisplay, CONTEXT *pContext)
{
    ...
    pRegDisplay->PC = (size_t)CORDbgGetIP(pContext);
    pRegDisplay->SP = (size_t)CORDbgGetSP(pContext);
    pRegDisplay->FP = (size_t)CORDbgGetFP(pContext);

#if _X86_
    pRegDisplay->Eax = pContext->Eax;
    pRegDisplay->Ebx = pContext->Ebx;
    pRegDisplay->Ecx = pContext->Ecx;
    pRegDisplay->Edx = pContext->Edx;
    pRegDisplay->Esi = pContext->Esi;
    pRegDisplay->Edi = pContext->Edi;
#elif defined(_PPC_) || defined(_SPARC_)
    // All that's needed for now
#else   // _X86_
    PORTABILITY_ASSERT("SetThreadContext doesn't know how to copy a context record on this platform.");
#endif  // _X86_
}
...
//          function is using.
// void *CodeVersionToken:  An opaque value to hand back to the left
