...
private:
    ...
    // The cache is implemented as a link list.                                                                
...
public:
    //
    // NOTE:  there must not be a constructor or destructor for
    // the MetadataPointerCache, as there is only one instance of the
    // class, which is stored in a global variable.  If this class has
    // a constructor, then it adds a dependency on the C Runtime
    // successfully constructing and destructing C++ objects.
    //
...
public:
    ...
    // a CordbModule. So if someone had a reference to a neutered module and tried to 
    // access the metadata cache in the CordbProcess an AV would occur.                        
    //
    // Therefore I've decided to make the cache static ensuring it's around if someone still
    // has a reference to a neutered CordbModule. 
    //
...
#ifndef RIGHT_SIDE_ONLY
    //                             modules are loaded sequentially, so we don't need a 
