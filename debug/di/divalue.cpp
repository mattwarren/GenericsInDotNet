    ...
    case RAK_REGREG:
        {
            ...
            if (SUCCEEDED(hr))
                hr = SetContextRegister(&c, m_remoteRegAddr.u.reg2, lowPart,
                                        frame);
        }
        break;
...
void CordbObjectValue::DiscardObject(void *token, bool fStrong)
{
    ...
    if (CORDBCheckProcessStateOK(m_process))
    {
        ...
        m_process->m_cordb->SendIPCEvent(m_process, &event,
                                                  sizeof(DebuggerIPCEvent));
...
// bool CordbObjectValue::SyncObject():  Obtains the most current info
...
// Returns true if the object is still valid & this instance has been
		...
		else 
		{
			if (m_remoteRegAddr.kind == RAK_REGREG)
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					LPVOID highWordAddr = pFrame->GetAddressOfRegister(m_remoteRegAddr.reg1);
					LPVOID lowWordAddr = pFrame->GetAddressOfRegister(m_remoteRegAddr.u.reg2);
