...
#ifdef _X86_
#include "../inc/i386/debugcpu.h"
#elif defined(_PPC_)
#include "../inc/ppc/debugcpu.h"
#else
...
public:
    ...
    void Do(DebuggerShell *shell, ICorDebug *cor, const WCHAR *args)
    {
        ...
        int iLength = wcslen (newPathArg);

        WCHAR *newPath = new WCHAR[iLength + 1];
        _ASSERTE(newPath != NULL);
        wcscpy(newPath, newPathArg);
...
static WCHAR *g_RegNames[REGISTER_X86_EFLAGS_OV+1] = {
...
static int g_numRegNames = NUMREGNAMES;
...
#else
            PORTABILITY_ASSERT("RegistersDebuggerCommand::Do (commands.cpp) is not implemented on this platform.");
#endif
        }
...
#define WRITE_REG(_shell, _val, _name, _tmp, _base) \
        ...
        case REGISTER_INSTRUCTION_POINTER:
            WRITE_REG(shell, (ULONG)CORDbgGetIP(pContext), g_RegNames[iReg], wszTemp, nBase);
            break;
        case REGISTER_STACK_POINTER:
            WRITE_REG(shell, (ULONG)CORDbgGetSP(pContext), g_RegNames[iReg], wszTemp, nBase);
            break;
        case REGISTER_FRAME_POINTER:
            WRITE_REG(shell, (ULONG)CORDbgGetFP(pContext), g_RegNames[iReg], wszTemp, nBase);
            break;
...
public:
    ...
    virtual void Do(DebuggerShell *shell, ICorDebug *cor, const WCHAR *args)
    {
        ...
        int i;
        for (i=0; i<(int)wcslen(szAddrTemp); i++)
		{
			if ((szAddrTemp[i] != L' ') &&
...
public:
	...
	HRESULT hr = PAL_CoCreateInstance(CLSID_CorpubPublish,
					  IID_ICorPublish,
					  (LPVOID *)&pPublish);
