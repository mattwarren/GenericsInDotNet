...
DebuggerModule::DebuggerModule(ICorDebugModule* imodule)
    : DebuggerBase((ULONG)imodule),
      m_pISymUnmanagedReader(NULL), m_sourceFiles(11), m_functions(37),
      m_functionsByIF(11), m_loadedClasses(11), m_breakpoints(NULL),
      m_szName(NULL)
{
...
DebuggerFunction::DebuggerFunction(DebuggerModule *m, mdMethodDef md,
    ...
    : DebuggerBase(md), m_module(m), m_class(0), m_ifunction(iFunction), 
      ...
      m_name(NULL), m_signature(NULL), m_namespaceName(NULL), m_className(NULL),
      m_VCHack(FALSE), m_arguments(NULL), m_argCount(0),
      m_returnType(NULL),
      m_symMethod(NULL), m_SPOffsets(NULL), m_SPDocuments(NULL), m_SPLines(NULL),
      m_SPCount(0),
                                           ...
                                           SIZE_T *rangeCount)
{
    ...
    *rangeCount = 0;
    ...
    // If we can get the ranges from the debug info
    if (file &&
        file->LoadText(g_pShell->m_currentSourcesPath, false) &&
        m_symMethod)
    {
    ...
    {
        // We want to step one IL instruction at a time
        rangeArraySize = 2;
        *range = new COR_DEBUG_STEP_RANGE[rangeArraySize / 2]; 
        _ASSERTE(*range != NULL);
        range[0]->startOffset = ip;
        range[0]->endOffset = ip+1;
    }
