...
HRESULT DebuggerShell::Init()
{
	...
	hr = PAL_CoCreateInstance(CLSID_CorDebug,
				  IID_ICorDebug,
				  (void **)&m_cor);
    ...
    if (expandObjects)
    {
        ...
        if (isValueClass)
        {
            ...
            if (SUCCEEDED(hr))
            {
                 ...
                 static const GUID IID_IManagedInstanceWrapper = {0x96E328FE,0x9A83,0x439F,{0xA2,0x98,0x8F,0x9C,0xAA,0x29,0x95,0xF0}};
                 IManagedInstanceWrapper* pmiw;
                 VARIANT retval;
                 HRESULT hr;
           
                 V_VT(&retval) = VT_EMPTY;
                 hr = pObject->QueryInterface(IID_IManagedInstanceWrapper, (void**)&pmiw);
                 if (SUCCEEDED(hr))
		 {
                    hr = pmiw->InvokeByName(L"ToString", CorFFIInvokeMethod, 0, NULL, &retval);

                    if (SUCCEEDED(hr) && V_VT(&retval) == VT_BSTR)
                    {
                       PrintIndent(indent + 1);
                       Write(L"\nObject::ToString(%s) = %s", name, V_BSTR(&retval));
                    }
                    else
                                        ...
                                        mdTypeDef *pTD)
{
         ...
         m = (DebuggerModule*) g_pShell->m_modules.FindNext(&find))
    {
        ...
        hr =
#endif
        FindTypeDefByName(m, className, &td);
...
HRESULT DebuggerShell::ResolveFullyQualifiedMethodName(const WCHAR *methodName, ICorDebugFunction **ppFunc)
{
    HRESULT hr = S_OK;
    *ppFunc = NULL;
...
//
BOOL DebuggerShell::ReadSourcesPath(WCHAR **currentPath)
{
    BOOL b;
    ...
    *currentPath = new WCHAR[wcslen(currentPathW)+1];
    if (!*currentPath) {
	return FALSE;
    }
    wcscpy(*currentPath, currentPathW);
    return TRUE;
}
...
// TRUE.
//
BOOL DebuggerShell::WriteSourcesPath(WCHAR *newPath)
{
    BOOL b;
                                                ...
                                                HANDLE hThread)
{
    ...
    HRESULT hr =
#endif
    g_pShell->m_unmanagedThreads.AddBase(ut);
    _ASSERTE(SUCCEEDED(hr));
}
...
BOOL DebuggerShell::UpdateCurrentPath (const WCHAR *newPath)
{
    ...
    if (iLength != 0)
    {
        ...
        HRESULT hr =
#endif
        m_FPCache.InitPathArray (m_currentSourcesPath);
...
DebuggerBreakpoint::DebuggerBreakpoint(const WCHAR *name, SIZE_T nameLength, 
    ...
    : m_index(index), m_threadID(threadID)
{
...
DebuggerBreakpoint::DebuggerBreakpoint(DebuggerFunction *f, 
    ...
    : m_index(offset), m_threadID(threadID)
{
    CommonCtor();
...
DebuggerBreakpoint::DebuggerBreakpoint(DebuggerSourceFile *file, 
    ...
    : m_index(lineNumber), m_threadID(threadID)
{
    CommonCtor();
        ...
        if (m_managed)
        {
            ...
            while (pCurNode != NULL)
            {
                    ...
                    (pCurNode->m_pModule->GetSymbolReader() != NULL))
                {
                    ...
                    WCHAR *tmpFilename = new WCHAR[filenameLength + 1];
                    _ASSERTE(tmpFilename != NULL);
                    wcscpy(tmpFilename, filename);
                    GUID g = {0};
                    ISymUnmanagedDocument *doc = NULL;
                    HRESULT hr = pCurNode->m_pModule->GetSymbolReader()->GetDocument(tmpFilename,
                                                                        g, g, g,
                                                                        &doc);
                    delete [] tmpFilename;
