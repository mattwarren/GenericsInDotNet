...
#ifdef USE_BP_TO_STEP
#define ShouldSingleStep(exception) (exception == EXCEPTION_BREAKPOINT)
#else
#define ShouldSingleStep(exception) (exception == EXCEPTION_SINGLE_STEP)
#endif
// -------------------------------------------------------------------------
...
// void DebuggerController::Uninitialize()    Does all the work
...
DebuggerController::DebuggerController(Thread *thread, AppDomain *pAppDomain)
  : m_pAppDomain(pAppDomain), m_thread(thread), m_singleStep(false),
    m_exceptionHook(false), m_traceCall(0), m_traceCallFP(FRAME_TOP),
    m_unwindFP(NULL), m_eventQueuedCount(0), m_deleted(false)
{
    ...
    if (patch->address == NULL)
    {
        ...
        if (code != NULL)
        {
            ...
            if (!patch->native)
            {
                ...
                LOG((LF_CORDB,LL_INFO10000, 
                    "DC::BindPa: Mapped ilOffset patch->offset 0x%x to native 0x%x\n",
...
bool DebuggerController::ApplyPatch(DebuggerControllerPatch *patch)
{
    ...
    LOG((LF_CORDB,LL_INFO10000, "DC::ApplyPatch at addr 0x%x\n", 
        patch->address));
    ...
    if (patch->native)
    {
        ...
        {   
            ...
            _ASSERTE( *(INSTRUCTION_TYPE *)patch->address == CORDbg_BREAK_INSTRUCTION );
            return true;
        }
...
bool DebuggerController::IsPatched(const BYTE *address, BOOL native)
{
    if (native)
    {
        return CORDbgIsPatchedAddress(address);
    }
    else
        return false;
}
...
// const BYTE * address:  The address that we want to 'see through'
...
// void DebuggerController::UnapplyPatchInCodeCopy()   Takes 
...
// void DebuggerController::UnapplyPatchesInMemoryCopy()  Same as 
...
// BYTE * memory:  Start address of the memory blob in which to unapply 
//      patches.  It's assumed that at this address is end-start bytes
//      of writable memory
// CORDB_ADDRESS start:  Start of the range of addresses.
// CORDB_ADDRESS end:  End of the range of addresses.
...
// bool DebuggerController::ReapplyPatchesInMemory()   For the 
...
// CORDB_ADDRESS start:  Beginning address of the range of memory
//      in which to reapply patches.
// CORDB_ADDRESS end:: End address of the range of memory in which
//      to reapply patches.
// Returns true if we were able to reapply all the patches in the
...
// BOOL DebuggerController::AddPatch()   What: Creates
...
// const BYTE *code:  Address of the first instruction of the code
...
static bool _AddrIsJITHelper(const BYTE *addr)
{
    ...
    if (addr >= pBase && addr < pBase + VAL32(pNT->OptionalHeader.SizeOfImage)) 
    {
        if (!tableLoaded)
        {
            ...
            HRESULT hr =
#endif
            LoadHelperTable(NULL, helperFuncs, CORINFO_HELP_COUNT);
            _ASSERTE(hr == S_OK);
...
bool DebuggerController::MatchPatch(Thread *thread, 
                                    ...
                                    DebuggerControllerPatch *patch)
{
    LOG((LF_CORDB, LL_INFO100000, "DC::MP: EIP:%p\n", GetIP(context)));
    ...
    _ASSERTE(patch->address == (const BYTE*)GetIP(context));
                                                         ...
                                                         BOOL fForEnC)
{
    ...
    DebuggerControllerPatch *patch = g_patches->GetPatch(PC);
    ...
    if (patch != NULL && patch->native)
    {
        ...
        //
        // !!! check result 
        LOG((LF_CORDB,LL_INFO10000, "DC::APS: About to skip from PC=0x%x\n", PC));
                                         ...
                                         TP_RESULT *pTpr)
{
           ...
           !fDone)
    {
        ...
        if (MatchPatch(thread, context, patch))
        {
            ...
            if (patch->deleted)
            {
                LOG((LF_CORDB, LL_INFO10000, "DC::SFT: patch deleted, deactivating\n"));
                DeactivatePatch(patch);
                g_patches->RemovePatch(patch);
            }
        }
        ...
        tpr != TPR_TRIGGER_ONLY_THIS)
    {
        ...
        while (p != NULL)
        {
            if (p->m_thread == thread && p->m_singleStep)
            {
#if !defined(USE_HARDWARE_TO_STEP)
...
//        LOG((LF_CORDB,LL_INFO1000, "DisablePreemptiveGC done!\n"));
        ...
        if (reabort)
        {
            if (::FuncEvalHijack != GetIP(context))
            {
#ifdef _DEBUG
                HRESULT hr =
#endif
                g_pDebugger->FuncEvalSetupReAbort(thread);
    ...
    if (tpr != TPR_TRIGGER_ONLY_THIS_AND_LOOP) // see DebuggerEnCRemap class
        ActivatePatchSkip(thread, (const BYTE*)GetIP(pCtx), FALSE);
...
void DebuggerController::ApplyTraceFlag(Thread *thread)
{
    ...
    if (context == NULL)
    {
        if (ISREDIRECTEDTHREAD(thread))
        {
            ...
            context = GETREDIRECTEDCONTEXT(thread);
        }
        else
        {
...
#elif defined(_PPC_)
    context->Msr |= 0x400UL;
#elif !defined(USE_BP_TO_STEP)
    PORTABILITY_ASSERT("ApplyTraceFlag (Controller.cpp) is not implemented on this platform.");
#endif
...
void DebuggerController::UnapplyTraceFlag(Thread *thread)
{
    ...
    if (context)
        context->Msr &= ~0x400UL;
#elif !defined(USE_BP_TO_STEP)
    PORTABILITY_ASSERT("UnApplyTraceFlag (Controller.cpp) is not implemented on this platform.");
#endif
...
// bool DebuggerController::DispatchAccessViolation()   DispatchAccessViolation 
                                                 ...
                                                 Thread *pCurThread)
{
    ...
    if (fDispatch)
    {
        ...
        case EXCEPTION_BREAKPOINT:
            ...
            if (result) 
            {
                _ASSERTE(CORDebuggerAttached() != 0);
            }
            break;
#endif //USE_BP_TO_STEP
    ...
    fDebuggers = (fDispatch?(result?true:false):true);
    ...
    m_address(patch->address)
{
    ...
    SIZE_T instructionLength = MAX_INSTRUCTION_LENGTH;
...
#ifdef USE_BP_TO_STEP
    // On the PowerPC, set the next instruction to trap
    CORDbgSetInstruction(m_patchBypass+sizeof(DWORD), CORDbg_BREAK_INSTRUCTION);
    LOG((LF_CORDB, LL_INFO10000,
         "DPS::DPS: Bypass BP at 0x%x = 0x%x\n", m_patchBypass+sizeof(DWORD), *m_patchBypass+sizeof(DWORD)));
#endif
    _ASSERTE( patch->opcode == 0 || patch->address != NULL);
    ...
    walker.Init(m_patchBypass, NULL);

    WALK_TYPE wt = walker.GetOpcodeWalkType();

    m_isCall = (wt == WALK_CALL);
    m_isAbsoluteBranch = walker.IsAbsoluteBranch();
                              ...
                              TRIGGER_WHY tyWhy)
{
    ...
    _ASSERTE((const BYTE*)GetIP(context) == GetBypassAddress());

    //We should be the _only_ patch here
    DebuggerControllerPatch *patchCheck = g_patches->
        GetPatch((const BYTE*)GetIP(context));
                                                  ...
                                                  EXCEPTION_RECORD *exception)
{
    ...
    if (m_isCall && ShouldSingleStep(exception->ExceptionCode))
    {
        // Fixup return address on stack

        DWORD *sp = (DWORD *) GetSP(context);
    ...
    if (!m_isAbsoluteBranch || !ShouldSingleStep(exception->ExceptionCode))
    {
        ...
        _ASSERTE(!IsBadReadPtr((const VOID *)GetIP(context), 1));
...
bool DebuggerStepper::ShouldContinueStep( ControllerStackInfo *info,
                                          SIZE_T nativeOffset)
{    
    LOG((LF_CORDB,LL_INFO10000, "DeSt::ShContSt: nativeOffset:0x%x \n", nativeOffset));
    if (m_rgfMappingStop != STOP_ALL && (m_reason != STEP_EXIT) )
    {
        ...
        if ( ji != NULL )
        {
            LOG((LF_CORDB,LL_INFO10000,"DeSt::ShContSt: For code 0x%x, got "
        ...
        {
            LOG((LF_CORDB,LL_INFO10000,"DeSt::ShContSt: For code 0x%x, didn't "
                "get DJI\n",(const BYTE*)*info->m_activeFrame.registers.pPC));
...
bool DebuggerStepper::TrapStep(ControllerStackInfo *info, bool in)
{
    ...
    LOG((LF_CORDB,LL_INFO1000, "DS::TS: GetJitInfo for pc = 0x%x (addr of "
        "that value:0x%x)\n", (const BYTE*)*info->m_activeFrame.registers.pPC,
    ...
    if( ji != NULL )
    {
        LOG((LF_CORDB,LL_INFO10000,"DS::TS: For code 0x%x, got DJI 0x%x, "
            "from 0x%x to 0x%x\n",
            (const BYTE*)*info->m_activeFrame.registers.pPC, 
            ji, ji->m_addrOfCode, ji->m_addrOfCode+ji->m_sizeOfCode));
    }
    ...
    {
        LOG((LF_CORDB,LL_INFO10000,"DS::TS: For code 0x%x, "
            "didn't get a DJI \n",
            (const BYTE*)*info->m_activeFrame.registers.pPC));
    }
    ...
    LOG((LF_CORDB,LL_INFO1000, "DS::TS: &info->m_activeFrame.registers 0x%x\n", &info->m_activeFrame.registers));

    // !!! Eventually when using the fjit, we'll want
    // to walk the IL to get the next location, & then map 
    // it back to native.
    walker.Init((BYTE*)*info->m_activeFrame.registers.pPC, &info->m_activeFrame.registers);
    ...
    if (info->m_activeFrame.fp == info->m_bottomFP)
    {
        ...
        do
        {
...
#ifdef _X86_
                    void * StackPointerAfterReturn = (void*)((DWORD)GetRegdisplaySP(&info->m_activeFrame.registers)
                                                    + sizeof(void*)); // return will pop at least the return address off the stack
                    if (info->m_activeFrame.fp > StackPointerAfterReturn )
                    {
                        LOG((LF_CORDB,LL_INFO10000,"DS::TS: intramethod return in fp:0x%08x with StackPointer:0x%08x\n",
                             info->m_activeFrame.fp, GetRegdisplaySP(&info->m_activeFrame.registers)));
#if defined(_DEBUG)
                        const BYTE *returnTo = *(BYTE**)GetRegdisplaySP(&info->m_activeFrame.registers);
...
#else
                    PORTABILITY_ASSERT("TrapStep WALK_Return not implemented on this platform");
                    if (IsAddrWithinMethod(ji, info->m_activeFrame.md, walker.GetNextIP()))
                    {
                        fDone = false;
                        wt = WALK_UNKNOWN;
            ...
            case WALK_CALL:
                ...
                {
                    LOG((LF_CORDB,LL_INFO100000, "DC::TS:Imm:WALK_CALL in\n"));
    ...
    while (TRUE)
    {
        ...
        case WALK_CALL:
            ...
            if (IsAddrWithinMethod(ji, info->m_activeFrame.md, walker.GetNextIP()))
            {
                LOG((LF_CORDB, LL_INFO10000, "DS::TS: WALK_CALL IsAddrWithinMethod, Adding Patch.\n"));
            ...
            if (in)
            {
                ...
                LOG((LF_CORDB, LL_INFO10000, "DS::TS: WALK_CALL Call TrapSetInfo\n"));
                if (TrapStepInto(info, walker.GetNextIP(), &td))
                {
                    ...
                    LOG((LF_CORDB, LL_INFO10000, "DS::TS: WALK_CALL TrapSetInfo returned true and we're returning true\n"));
                    return true;
                }
            }
            ...
            if (walker.GetSkipIP() == NULL)
            {
                LOG((LF_CORDB, LL_INFO10000, "DS::TS: WALK_CALL GetSkipIP, Adding Patch\n"));
                AddPatch(info->m_activeFrame.md, 
                         offset,
                         TRUE, 
                         info->m_returnFrame.fp, 
                                    ...
                                    CorDebugStepReason unwindReason)
{
    ...
    //
    // An exception is blowing our stepping all to hell.  Set a patch on 
    // the filter/handler.
    // 
...
void DebuggerFuncEvalComplete::SendEvent(Thread *thread)
{
    ...
    DebuggerEval *pDE = (DebuggerEval*)(GetIP(pCtx));
