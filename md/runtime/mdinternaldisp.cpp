    ...
    MDFileFormat *pFormat)                  // [OUT] the file format
{
    ...
    *pFormat = MDFormat_Invalid;
    ...
    if (FAILED(hr = MDFormat::VerifySignature((STORAGESIGNATURE *) pData)))
        goto ErrExit;
...
extern "C" 
{
HRESULT FindImageMetaData(PVOID pImage, PVOID *ppMetaData, ULONG *pcbMetaData, DWORD dwFileLength);
}
    ...
    IUnknown    **ppIUnk)               // [out] Return interface on success.
{
    ...
    DWORD dwFileLength;
    AssemblyMDInternalImport *pAssemblyMDInternalImport;
    ...
    case CorReLoadOSMap:
        {
            ...
            pData = VAL32(pCorHeader->MetaData.VirtualAddress) + (PBYTE) base;
            cbData = VAL32(pCorHeader->MetaData.Size);
            break; 
        }
    default:
    ...
    pAssemblyMDInternalImport = new AssemblyMDInternalImport (pMDInternalImport);
...
AssemblyMDInternalImport::~AssemblyMDInternalImport () 
{
    ...
    else if(m_pHandle) 
    {
        ...
        HRESULT hr =
#endif
        RuntimeReleaseHandle(m_pHandle);
...
STDMETHODIMP AssemblyMDInternalImport::EnumGenericPars(HCORENUM *phEnum, mdToken tkOwner,
        mdGenericPar rTokens[], ULONG cTokens, ULONG *pcTokens)
{
    _ASSERTE(!"NYI");
    return E_NOTIMPL;
}

STDMETHODIMP AssemblyMDInternalImport::GetGenericParProps(
