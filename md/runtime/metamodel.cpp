    ...
    const void      *pvData)            // Data to load from.
{
    ...
    // Transfer the fixed fields.
    memcpy(static_cast<CMiniMdSchemaBase*>(this), static_cast<const CMiniMdSchemaBase*>(pvData), sizeof(CMiniMdSchemaBase));
    ...
    {
        m_ulExtra = (VAL32(*reinterpret_cast<const ULONG*>(&pSource->m_cRecs[iSrc])));
        ulData += sizeof(ULONG);
    }
