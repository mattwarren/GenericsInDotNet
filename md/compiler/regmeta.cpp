...
HRESULT RegMeta::StartupEE()
{
    ...
    PAL_TRY
    {
        DWORD   dwBuffer[(sizeof(DWORD) + (MAX_PATH+1) * sizeof(WCHAR) + sizeof(DWORD)-1) / sizeof(DWORD)];
        BSTR    bstrDir = NULL;
        IfFailGo(PAL_CoCreateInstance(CLSID_CorRuntimeHost,
                                        IID_ICorRuntimeHost,
                                        (void**)&m_pCorHost));
    ...
    mdMethodDef rmdOtherMethods[])      // [IN] Array of other methods.
{
    ...
    // Store the getter.
    if (!IsNilToken(mdGetter))
    {
        _ASSERTE(TypeFromToken(mdGetter) == mdtMethodDef);
        IfFailGo(_DefineMethodSemantics(msGetter, mdGetter, pr, bClear));
    }

    // Store the setter.
    if (!IsNilToken(mdSetter))
    {
        _ASSERTE(TypeFromToken(mdSetter) == mdtMethodDef);
